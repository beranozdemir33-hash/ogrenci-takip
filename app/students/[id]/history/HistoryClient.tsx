...
export default function HistoryClient({ studentId, studentName, lessons }: HistoryClientProps) {
    const reportRef = useRef<HTMLDivElement>(null);

    const getAttendanceLabel = (status: string) => {
        switch (status) {
            case "completed": return { text: "Geldi", color: "text-green-600 bg-green-100 dark:bg-green-900/30 dark:text-green-400" };
            case "cancelled": return { text: "İptal", color: "text-red-600 bg-red-100 dark:bg-red-900/30 dark:text-red-400" };
            case "scheduled": return { text: "Planlandı", color: "text-orange-600 bg-orange-100 dark:bg-orange-900/30 dark:text-orange-400" };
    ...
    const downloadPDF = async () => {
        ...
        const canvas = await html2canvas(reportRef.current, {
            ...
            ignoreElements: (element) => element.classList.contains('no-print') // Ignore delete buttons in PDF
        });
        ...
        pdf.save(`ders_gecmisi_${studentName.replace(/\s+/g, '_')}.pdf`);
    };
                ...
                <div ref={reportRef} className="w-[595px] min-h-[842px] bg-white p-8 text-black font-sans">
                    <div className="flex items-center justify-between border-b-2 border-gray-800 pb-4 mb-8">
                        ...
                        <div className="text-right">
                            <h2 className="text-xl font-bold text-gray-800">DERS GEÇMİŞİ RAPORU</h2>
                    ...
                    <table className="w-full text-left text-sm border-collapse">
                        <thead>
                            <tr className="bg-gray-100 border-b-2 border-gray-300">
                                ...
                                <th className="p-3 font-bold text-gray-700">Konu</th>
                                <th className="p-3 font-bold text-gray-700">Ödev</th>
                                <th className="p-3 font-bold text-gray-700 text-center">Durum</th>
                            </tr>
                        </thead>
                        <tbody>
                            {lessons.map((lesson, i) => (
                                <tr key={lesson.id} className={clsx("border-b border-gray-200", i % 2 === 0 ? "bg-white" : "bg-gray-50")}>
                                    <td className="p-3 text-gray-800">{formatDate(lesson.date)}</td>
                                    <td className="p-3 text-gray-800">{lesson.subject}</td>
                                    <td className="p-3 text-gray-600 max-w-[150px] truncate">{lesson.homework || "-"}</td>
                                    <td className="p-3 text-center">
                                        <span className={clsx("inline-block px-2 py-1 rounded text-xs font-bold",
                                            lesson.status === 'completed' ? 'bg-green-100 text-green-800' :
                                                lesson.status === 'cancelled' ? 'bg-red-100 text-red-800' : 'bg-orange-100 text-orange-800')}>
                                            {getAttendanceLabel(lesson.status).text}
                                        </span>
                                    </td>
                                </tr>
                            ))}
                        ...
                        <table className="w-full text-left text-sm text-text-secondary-light dark:text-text-secondary-dark">
                            <thead className="bg-gray-50 dark:bg-gray-800 text-xs uppercase text-text-secondary-light dark:text-text-secondary-dark">
                                <tr>
                                    ...
                                    <th scope="col" className="px-4 py-3 font-medium">Konu</th>
                                    <th scope="col" className="px-4 py-3 font-medium">Ödev</th>
                                    <th scope="col" className="px-4 py-3 font-medium text-center">Durum</th>
                                    <th scope="col" className="px-4 py-3 font-medium text-right">İşlem</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-border-light dark:divide-border-dark">
                                {lessons.map((lesson) => {
                                    const attendance = getAttendanceLabel(lesson.status);
                                    return (
                                        <tr key={lesson.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                            ...
                                            <td className="px-4 py-3 max-w-[150px] truncate" title={lesson.homework}>
                                                {lesson.homework || "-"}
                                            </td>
                                            <td className="px-4 py-3 text-center">
                                                <span className={clsx("inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium", attendance.color)}>
                                                    {attendance.text}
                                                </span>
                                            </td>
                                            <td className="px-4 py-3 text-right">
                                                <button
                                                    ...
                                                    className="inline-flex h-8 w-8 items-center justify-center rounded-full text-text-secondary-light hover:bg-red-50 hover:text-red-600 dark:text-text-secondary-dark dark:hover:bg-red-900/20"
                                                >
